$(document).ready(function () {
    ...
    $('input#open, input#expired').datetimepicker();

    $(document).on('change', "select[name='city_id']", function() {
        $('select#editors option').remove();
        $('select#district option').remove();
        $('select#institutions option').remove();

        $.ajax({
            url: '/api/editors/get-by-city',
            data: {city_id: $(this).val()},
            success: function (response) {
                response.data.forEach(function(item) {
                    $('select#editors').append('<option value="'+item.id+'">'+item.name+'</option>')
                });
            },
            dataType: 'json'
        });
        ...
        $.ajax({
            url: '/api/institutions/get-by-city',
            data: {city_id: $(this).val()},
            success: function (response) {
                response.data.forEach(function(item) {
                    $('select#institutions').append('<option value="'+item.id+'">'+item.name+'</option>')
                });
            },
            dataType: 'json'
        });
